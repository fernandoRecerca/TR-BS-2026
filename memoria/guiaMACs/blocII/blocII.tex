...
``La \textbf{població} o \textbf{l'univers} és el conjunt de tots els individus que són objecte del nostre estudi.

\textbf{Una mostra} és un subconjunt extret de la població. El seu estudi serveix per inferir característiques de tota la població.


\subsubsection{Mostreig}
En substituir l'estudi de la població pel de la mostra, es cometen errors. Però hi comptem per endavant i poden controlar-se.

Tanmateix, si la mostra està mal triada (no és \textbf{representativa}) es produeixen errors addicionals imprevistos i incontrolables (\textbf{biaixos}).

L'elecció de la mostra s'anomena \textbf{mostreig}. Vegem a continuació com s'ha de realitzar el mostreig perquè ens proporcioni mostres representatives.
...
Per exemple, imagina que en un centre escolar volen saber el temps que dediquen a estudiar, de mitjana, els 1\,300 alumnes i per a això es decideix extreure una mostra de 100 d'ells.

\begin{enumerate}[a)]
 \item Si fos el director o una comissió de professors els que triessin els estudiants, procurant que hi hagués estudiants «bons», «mitjans», «fluixos»..., la mostra seria esbiaixada, ja que no reflectiria la realitat sinó el que el director o els professors creguessin veure de la realitat.

 \item Si triessin els 100 primers alumnes que arriben al centre un cert dia, la mostra també estaria \emph{contaminada}, perquè és possible que el fet «d'arribar aviat al centre» tingui alguna cosa a veure amb el grau de responsabilitat d'aquests alumnes i, per tant, amb la seva dedicació a l'estudi.

 \item Si els 100 individus de la mostra se seleccionen mitjançant sorteig (es trien a l'atzar), llavors sí que serà representativa. Aquest mostreig s'anomena \emph{aleatori}.
\end{enumerate}
...
És el tipus de mostreig aleatori més senzill i tots els altres hi estan basats. Per obtenir una mostra, es numeren els elements de la població i se seleccionen a l'atzar els $n$ elements que ha de contenir la mostra. Si els individus són, per exemple, els cargols que hi ha en una capsa, per obtenir la mostra n'hi ha prou amb prendre'n $n$ per simple extracció.
